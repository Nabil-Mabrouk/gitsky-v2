...
Un principe non négociable structure toute l'intégration : **l'IA est non-déterministe, mais GitSky exige la reproductibilité.** Un `config.yaml` doit toujours regénérer le même projet — c'est ce qui rend le cycle kill → résurrection du Chap 20 possible.
