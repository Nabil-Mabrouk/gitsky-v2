...
> **Écart assumé (round layout)** — troisième catégorie, ni châssis pur ni vitrine gelée : `frontend/src/components/layout/{Navbar,Footer}.tsx` sont générés avec des valeurs par défaut minimales puis **destinés à être personnalisés en local** par le développeur (avec l'IA de son choix) une fois le projet cloné — le cycle de vie que documente `AGENTS.md`/`CLAUDE.md` (généré à la racine de chaque projet, non conditionné aux modules actifs comme l'est `MODULES.md`). Ce ne sont pas des fichiers `_skip_if_exists` : un `copier update` continue de les fusionner à 3 voies comme le reste du châssis — diverger dessus est le comportement *attendu*, pas une exception à documenter au cas par cas.
>
> Autre changement du même round : la racine `/` affiche **toujours** la landing, quels que soient les modules actifs sur le projet — y compris pour le fleet dashboard lui-même. Avant, `App.tsx` redirigeait `/` vers `/learn` dès qu'un module au-delà de `auth` était actif ; activer `MODULE_ADMIN` sur un projet faisait ainsi disparaître sa landing, un vrai bug de prod découvert sur politique-ia. `App.tsx` déclare désormais deux groupes de routes indépendants (`/` → `Landing`, `/*` → le reste de l'app) plutôt qu'un branchement conditionnel sur les modules.

> **Écart assumé (round theming)** — retour d'expérience d'un développeur ayant réellement suivi le cycle de vie décrit ci-dessus (cloner, personnaliser avec une IA, `AGENTS.md`) : le round layout n'avait branché `Navbar`/`Footer` que sur `Landing.tsx` — `AppShell` (Login/Apprendre/Admin) gardait sa propre nav Tailwind brute, sans marque, sans `landing.css`. Une personnalisation de la landing ne se voyait donc nulle part ailleurs. `AppShell` réutilise désormais le même `Navbar`/`Footer` (autonomes : un seul `GET /health` fournit `project` et `modules`, plus besoin de props). Seule la règle de fond (`color`/`background`/`line-height`) est partagée entre `.landing` et le nouveau `.site-shell` — les règles plus spécifiques (titres, sections, badges) restent propres à la landing ; le contenu routé garde ses styles Tailwind. L'intérieur d'Admin (`admin-theme.css`, tokens `--admin-*`) reste **délibérément** neutre — `.admin-shell` pose son propre fond opaque par-dessus `.site-shell`, décision produit assumée (beaucoup de back-offices restent volontairement neutres plutôt que brandés), pas une incohérence à corriger.
>
> Même round : `frontend/src/branding.local.css`, importé après `theme.css.jinja` dans `index.css`. `theme.css` reste généré depuis le branding du Studio (`copier update` le régénère) — mais aucun fichier de config de branding n'était éditable localement une fois le projet cloné et développé indépendamment du Studio qui l'a créé. `branding.local.css` comble ce trou avec le même patron que `.env.local` (Chap 23) et `Navbar.tsx`/`Footer.tsx` (round layout) : généré vide, jamais re-templaté, une redéfinition de `--color-primary` y gagne automatiquement (dernière déclaration de la custom property sur `:root`).
