...
A **French-language training curriculum** teaching professional use of Claude Code. It is **content only** — no code, no build system, no tests. All material lives in `docs/formation-claude-code/` as Markdown files.
...
## No build / no tests (docs/formation-claude-code only)

For the training curriculum in `docs/formation-claude-code/` there is nothing to compile or run. "Verifying a change" means: read the edited Markdown end-to-end, check internal links resolve, and confirm the module still fits its stated duration.

## GitSky code development (docs/Template-book/ + src/)

The repo also hosts the **GitSky** startup-factory project: the book lives in `docs/Template-book/` and its implementation code is being built out. These rules apply to all GitSky code work:
...
- **The book is the source of truth.** If development diverges from the plan as described in the book (`docs/Template-book/`), stop and ask for the user's opinion *before* changing the book to match. Do not silently update the book to paper over a divergence.
